      ...
      {mounted && open && !isMobile && createPortal(
        <>
          ...
          <div className="fixed z-[9999] bg-white border rounded-2xl shadow-2xl p-2" style={{ top: pos.top, left: pos.left, width: pos.width }}>
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search a city, hotel, landmark…"
      ...
      {mounted && showDest && isMobile && createPortal(
        <div className="fixed inset-0 z-[10000]">
          ...
          <div className="absolute inset-x-0 bottom-0 top-12 bg-white rounded-t-2xl shadow-2xl p-4 overflow-y-auto">
            ...
            <input autoFocus value={destQuery} onChange={(e) => setDestQuery(e.target.value)} placeholder="Search a city, hotel, landmark…"
                   className="w-full text-[16px] pb-2 border-b border-gray-300 outline-none placeholder:text-gray-400" />
            <div className="mt-4 flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 cursor-pointer" onClick={()=>{
              if (!navigator.geolocation) return;
              navigator.geolocation.getCurrentPosition(async (p) => {
                const { latitude, longitude } = p.coords;
                try {
              ...
              <div><div className="text-sm font-medium text-gray-900">Use current location</div><div className="text-xs text-gray-500">Use device location</div></div>
            </div>
...
export default function SearchResultsPage() {
    ...
    return (
      <button
        ...
        className={[
          ...
          between ? 'bg-blue-100' : '',
          selectedStart || selectedEnd ? 'bg-[#111] text-white font-semibold' : (muted ? 'text-gray-300' : 'text-gray-800'),
        ].join(' ')}
  ...
  const summaryLabel = `${roomsCount} ${roomsCount > 1 ? 'Rooms' : 'Room'} • ${adults} ${adults > 1 ? 'Adults' : 'Adult'} • ${children} ${children !== 1 ? 'Children' : 'Child'}${infants>0?` • ${infants} Infant${infants>1?'s':''}`:''}`;
    ...
    const run = async () => {
      ...
      try {
        ...
        const qs = new URLSearchParams({
          startDate, endDate, lng, lat,
          adult: String(adults), child: String(children), infant: '0', pet: 'no',
        });
        ...
        else { setRooms([]); setFetchError('No availability found.'); }
      } catch {
  ...
  const applySearch = () => {
    const query = new URLSearchParams({
      startDate, endDate,
      adult: String(adults), child: String(children),
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 md:py-6">
      ...
      {/* ===== Search pill styled like your screenshot ===== */}
      <div className="w-full bg-white rounded-[1.25rem] md:rounded-[2rem] shadow-xl border border-gray-200 px-4 py-3 md:px-6 md:py-4 mb-6">
        {!isMobile ? (
          <div className="flex items-center gap-4">
            ...
            {/* Rooms & Guests summary + Search button */}
            <div className="flex items-center gap-4 flex-1">
              <div className="flex-1" ref={guestsRef}>
                <div className="flex items-center gap-2 text-[#F05A28] uppercase tracking-wide text-[10px] font-semibold mb-1">
                  <UsersIcon className="w-4 h-4" /> Rooms & Guests
                </div>
                <button type="button" onClick={() => setShowGuests(true)} className="w-full text-left text-lg md:text-[16px] font-medium text-gray-900">
                  {summaryLabel}
                </button>
              </div>
              ...
              <button
                ...
                className={`font-semibold tracking-wide px-7 py-3 rounded-full transition ${
                  dest && checkIn && checkOut ? 'bg-[#F05A28] hover:brightness-95 text-white' : 'bg-[#F05A28] text-white/70 cursor-not-allowed'
                }`}
                type="button"
              >
                SEARCH
          ...
          // Mobile: compact split button (unchanged)
          <div className="col-span-2">
            <button type="button" className="w-full flex items-center bg-white border border-gray-300 rounded-xl overflow-hidden shadow-sm">
              {/* Destination */}
              <div className="flex-1 px-4 py-3 text-left hover:bg-gray-50 cursor-pointer" onClick={() => window.dispatchEvent(new Event('open-dest-modal'))}>
                <div className="flex items-center gap-1 text-[#F05A28] text-[10px] uppercase font-semibold tracking-wide">
                  <PinIcon className="w-3.5 h-3.5" /> Destination
                </div>
                <div className="text-sm font-medium text-gray-900 truncate">{dest?.label || 'Where next?'}</div>
              </div>
              <div className="w-px bg-gray-300 self-stretch" />
              {/* Dates */}
              <div className="flex-1 px-4 py-3 text-left hover:bg-gray-50 cursor-pointer" onClick={(e) => { e.stopPropagation(); setShowCal(true); }}>
                <div className="flex items-center gap-1 text-[#F05A28] text-[10px] uppercase font-semibold tracking-wide">
      ...
      {!isMobile && showCal &&
        createPortal(
          <>
            ...
            <div className="fixed z-[9999] bg-white border rounded-2xl shadow-2xl p-4 w-[860px]" style={{ top: calPos.top, left: calPos.left }}>
              ...
              <div className="flex items-center justify-between mt-3">
                <button className="text-sm text-gray-700 hover:underline" onClick={() => { setCheckIn(null); setCheckOut(null); }}>
                  Reset
                </button>
                ...
                <div className="flex items-center gap-2">
                  <button className="text-sm px-4 py-2 rounded-lg border hover:bg-gray-50" onClick={() => setShowCal(false)}>Close</button>
                  <button
                    disabled={!checkIn || !checkOut}
                    className={`text-sm px-4 py-2 rounded-full text-white ${checkIn && checkOut ? 'bg-[#111]' : 'bg-gray-300 cursor-not-allowed'}`}
      ...
      {!isMobile && showGuests &&
        createPortal(
          <>
            ...
            <div className="fixed z-[9999] bg-white border rounded-xl shadow-2xl p-3 w-[380px]" style={{ top: guestPos.top, left: guestPos.left }}>
              {[
                ...
                <div key={row.label} className="flex items-center justify-between py-3 border-b last:border-b-0">
                  ...
                  <div className="flex items-center gap-3">
                    <button className="w-8 h-8 rounded-full border text-gray-700 hover:bg-gray-50 disabled:opacity-40"
                            onClick={() => step(row.setter as any, row.value as number, -1, (row as any).min, (row as any).max)}
                            disabled={(row.value as number) <= (row as any).min}>−</button>
                    <div className="w-5 text-center">{row.value}</div>
                    <button className="w-8 h-8 rounded-full border text-gray-700 hover:bg-gray-50 disabled:opacity-40"
                            onClick={() => step(row.setter as any, row.value as number, +1, (row as any).min, (row as any).max)}
                            disabled={(row.value as number) >= (row as any).max}>+</button>
                  </div>
                </div>
              ))}
              <div className="flex justify-end gap-2 pt-2">
                <button className="px-4 py-2 rounded-lg text-black border hover:bg-gray-50" onClick={() => setShowGuests(false)}>Close</button>
      ...
      <div className="space-y-6">
        {rooms.map((room: any) => {
          const nightsCount = checkIn && checkOut ? Math.max(1, differenceInCalendarDays(checkOut, checkIn)) : 0;
          ...
          return (
            <div key={`${room.roomTypeId}-${room.hotelId}-${room.hotelName}`} className="flex flex-col md:flex-row border rounded-xl p-4 shadow-sm bg-white">
              {/* Image */}
              <div className="md:w-72 w-full md:mr-6 mb-3 md:mb-0">
                {imgSrc ? (
                  <div className="relative w-full h-44 md:h-44 rounded-lg overflow-hidden bg-gray-100">
                    <Image src={imgSrc} alt={room.hotelName || 'Hotel'} fill className="object-cover" priority />
                  </div>
                ) : (
              ...
              <div className="flex-1 flex flex-col md:flex-row">
                <div className="flex-1 pr-0 md:pr-6">
                  <h2 className="text-[18px] md:text-[20px] font-semibold text-gray-900 mb-1">{room.hotelName}</h2>
                  <p className="text-sm text-gray-700 mb-1">Room Type: <span className="font-medium text-gray-900">{room.RoomType}</span></p>
                  <p className="text-sm text-gray-700 mb-1">Capacity: <span className="font-medium text-gray-900">{room.capacity} guest{room.capacity>1?'s':''}</span></p>
                  <p className="text-sm text-gray-700 mb-2">
                    {nightsCount} {nightsCount === 1 ? 'night' : 'nights'} — Total: <span className="font-semibold text-gray-900">${total.toFixed(2)}</span>
                    {nightsCount > 0 && <span className="text-gray-500"> ({nightly.toFixed(2)} / night)</span>}
                  </p>
                </div>

                <div className="mt-3 md:mt-0 md:w-56 flex md:flex-col md:items-end items-start gap-3">
                  <div className="md:text-right">
                    <div className="text-[22px] md:text-[24px] font-semibold text-gray-900 leading-none">${nightly.toFixed(2)}</div>
                  ...
                  <button
                    onClick={() => {
                      const q = new URLSearchParams({
                        ...
                        startDate, endDate, adult: String(adults), child: String(children),
                      });
